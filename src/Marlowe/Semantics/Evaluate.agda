...
evaluate _ s (AvailableMoney a t) = fromMaybe 0 ((a , t) ‼ (State.accounts s))
  where open Decidable _eqAccountIdTokenDec_ using (_‼_)
...
evaluate _ s (ChoiceValue c) = fromMaybe 0 (c ‼ State.choices s)
  where open Decidable _eqChoiceId_ using (_‼_)
...
evaluate _ s (UseValue v) = fromMaybe 0 (v ‼ (State.boundValues s))
  where open Decidable _eqValueId_ using (_‼_)
...
observe _ s (ChoseSomething c) = ⌊ c ∈? (State.choices s) ⌋
  where open Decidable _eqChoiceId_ using (_∈?_)
...
observe e s (ValueEQ x y) = ⌊ evaluate e s x ℕ.≟ evaluate e s y ⌋
